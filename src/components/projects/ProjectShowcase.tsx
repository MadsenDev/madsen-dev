...
import { ProjectShowcaseContent, ShowcaseIcon } from '@/data/projectPages';
import { useLanguage } from '@/contexts/LanguageContext';

const iconComponents: Record<ShowcaseIcon, ComponentType<{ className?: string }>> = {
  sparkles: Sparkles,
  bolt: Bolt,
  layers: Layers,
  map: MapPin,
      ...
      case 'device':
        return (
          <div className={`${baseClasses} aspect-[16/10] bg-gradient-to-br from-white/5 to-white/0`}>
            ...
            <div className="absolute inset-[1.75rem] rounded-xl overflow-hidden">
              {image}
            </div>
          </div>
        );
  ...
  return (
    <div className="space-y-24 pb-24">
            ...
            <div className="grid gap-10 lg:grid-cols-[1.1fr_1fr]">
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                {section.features.map((feature) => {
                  ...
                  return (
                    <div key={feature.title} className="group rounded-2xl border border-white/10 bg-slate-950/40 p-5 transition hover:border-white/25 hover:bg-slate-950/60">
      ...
      <section className="rounded-3xl border border-white/10 bg-white/5 p-10 backdrop-blur-md">
        <h2 className="text-2xl font-semibold text-white">{t('projects.technologiesUsed') ?? 'Technologies used'}</h2>
        <div className="mt-6 flex flex-wrap gap-3">
          {project.technologies.map((tech) => (
            <span
              key={tech}
              className="rounded-full border border-white/10 bg-slate-950/40 px-4 py-2 text-sm text-white/70"
            >
              {tech}
            </span>
          ))}
